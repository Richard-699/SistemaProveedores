...
if ($opcion == 'Cumplimiento') {
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Rechazo Cumplimiento';
            ...
            <h4 style="margin-top: 20px;">Cumplimiento ha rechazado el formato de vinculación LAFT de ' . $nombre_proveedor . '.</h4>
            <hr style="background-color: #cccccc; border: none; height: 1px; width: 100%; margin-top: 20px; margin-bottom: 20px;">
                <div style="width: 95%; text-align: justify; margin-left: auto; margin-right: auto;">
                    <div>
                        <p><span style="font-weight: bold;">Puedes validar el rechazo aquí: </span><a href="https://sistema.proveedores.hacebwhirlpoolindustrial.com/SistemaProveedores/index.php">Haceb Whirlpool</a></p>
...
if ($opcion == 'Negociacion') {
    ...
    $stmt = $conexion->prepare("UPDATE proveedores SET proveedor_aprobado = ? WHERE Id_proveedor = ?");
    $stmt->bind_param('ss', $proveedor_aprobado, $id_proveedor);
    if ($stmt->execute()) {
        $response = ['success' => true];
    } else {
        $response = ['success' => false, 'message' => 'Error al aprobar negociación.'];
